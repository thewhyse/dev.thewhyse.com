...
/**
 * Class to make queries to the Stabble Diffusion API
 */
class StabbleDiffusion extends Singleton {
	...
	public function get_api( $token = '' ) {
		if ( empty( $this->api ) ) {
			...
			if ( ! empty( $token ) ) {
				$this->api = new \StabbleDiffusion\Api\Images( $token );
			}
		}
		return $this->api;
	}
	...
	protected function get_guidance_scale() {
		return trx_addons_get_option( 'ai_helper_guidance_scale_stabble_diffusion', 7.5 );
	}
	 ...
	 * @access protected
	 * 
	 * @return float  Interference steps for the API
	 */
	protected function get_interference_steps() {
		return trx_addons_get_option( 'ai_helper_interference_steps_stabble_diffusion', 21 );
	}
	...
	public function generate_images( $args = array() ) {
		$args = array_merge( array(
			...
			'n' => 1,
			'model' => '',
			'num_inference_steps' => $this->get_interference_steps(),
			'guidance_scale' => $this->get_guidance_scale()
		), $args );
