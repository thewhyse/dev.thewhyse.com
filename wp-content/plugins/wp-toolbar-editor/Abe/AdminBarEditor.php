...
class Abe_AdminBarEditor {
    ...
    public function __construct() {
		...
		add_action('admin_menu_editor-register_hideable_items', array($this, 'registerHideableItems'), 10, 1);
	    add_filter(
			'admin_menu_editor-save_hideable_items-' . self::HIDEABLE_COMPONENT_ID,
			array($this, 'saveHideableItems'), 10, 2
	    );
	...
	private static function addHideableItem($store, $node, $category, $allNodes, $allRoles) {
		...
		$enabled = isset($node->is_visible_to_actor) ? $node->is_visible_to_actor : array();
		if ( $node->is_hidden ) {
	...
	public function saveHideableItems($errors, $items) {
		...
		foreach ($customNodes as $node) {
			...
			if ( isset($items[$id]) ) {
				$newEnabled = isset($items[$id]['enabled']) ? $items[$id]['enabled'] : array();
				$oldEnabled = isset($node->is_visible_to_actor) ? $node->is_visible_to_actor : array();
				$nodeModified = false;
