jQuery(document).ready(function($) {
	...
	for( let i = 0; i < ml_module_names.length; i++ ) {
		$( document ).on( 'change', '[data-settings-group="ml-row"] select', function() {

			let current_module_name = $( this ).val();

			update_ml_module_mark();
			...
			// Unassign module from the previous row
			$( '[data-settings-group="ml-row"] select' ).each( function() {
				if ( current_input_name !== $( this ).attr( 'name' ) && $( this ).val() === current_module_name ) {
	...
	function update_ml_module_mark() {

		let assigned_modules = [];
		$( '[data-settings-group="ml-row"] select' ).each( function() {
			if ( $( this ).val() !== 'none' ) {
				assigned_modules.push( {
					row_label: $( this ).closest( '.epkb-admin__form-tab-content' ).data( 'ml-row-label' ),
					module_name: $( this ).val(),
				} );
			}
		} );

		// Add green mark to each module that is assigned to any row
		$( '.epkb-input-custom-dropdown__option-mark' ).html( '' );
		for ( let i = 0; i < assigned_modules.length; i++ ) {
			$( '[data-value="' + assigned_modules[i].module_name + '"]:not(.epkb-input-custom-dropdown__option--selected)' ).find( '.epkb-input-custom-dropdown__option-mark' ).html( '(' + assigned_modules[i].row_label + ')' );
		}
	}

	// Initialize Module lists
	$( '[data-settings-group="ml-row"] select' ).trigger( 'change' );
	...
	// SHow/Hide article couner settings based on counting active/disable
