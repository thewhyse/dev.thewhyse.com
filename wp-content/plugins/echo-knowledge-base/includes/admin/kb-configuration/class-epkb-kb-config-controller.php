...
class EPKB_KB_Config_Controller {
	...
	public function apply_settings_changes() {
		...
		// save Modular Layout custom CSS if defined
		if ( isset( $new_config['epkb_ml_custom_css'] ) ) {
			...
			$result = EPKB_Utilities::save_wp_option( 'epkb_ml_custom_css_' . $kb_id, $ml_custom_css );
			if ( is_wp_error( $result ) ) {
				EPKB_Utilities::ajax_show_error_die( EPKB_Utilities::report_generic_error( 35, $result ) );
			}
		}

		// store categories list as text with comma separated ids for Modular Layout FAQs Module
		$new_config['ml_faqs_category_ids'] = isset( $new_config['ml_faqs_category_ids'] ) ? implode( ',', $new_config['ml_faqs_category_ids'] ) : '';
