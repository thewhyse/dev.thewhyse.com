...
class EPKB_KB_Config_Layouts {
	...
	public static function get_main_page_layout_name_value() {
		$core_layouts = array (
			...
			// EPKB_KB_Config_Layout_Modular::LAYOUT_NAME  => __( 'Modular', 'echo-knowledge-base' ),
		);

		if ( EPKB_Utilities::is_elegant_layouts_enabled() ) {
			return array_merge( $core_layouts, array(
				EPKB_KB_Config_Layouts::GRID_LAYOUT     => __( 'Grid', 'echo-knowledge-base' ),
				EPKB_KB_Config_Layouts::SIDEBAR_LAYOUT  => __( 'Sidebar', 'echo-knowledge-base' ),
			) );
		}
	...
	public static function get_layout_mapping() {
		$core_layouts = array (
			...
			array( EPKB_KB_Config_Layout_Modular::LAYOUT_NAME => self::KB_ARTICLE_PAGE_NO_LAYOUT ),
		);
