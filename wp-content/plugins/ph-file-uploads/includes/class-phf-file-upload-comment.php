...
class PHF_File_Upload_Comment {
	...
	public function save_comment_attachments( $value, $comment, $attr, $request, $object_type ) {
		...
		$post_type = get_post_type( $value[0] );

		if( 'attachment' == $post_type ) {
			$is_comment_private = get_comment_meta( $comment->comment_ID, 'is_private', true );
			
			if( "true" == $is_comment_private ) {
				update_post_meta( $value[0], 'is_private_comment', $is_comment_private );
			}
		}
