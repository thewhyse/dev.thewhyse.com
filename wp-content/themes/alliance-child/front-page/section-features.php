<div class="front_page_section front_page_section_features<?php
	$alliance_scheme = alliance_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $alliance_scheme ) && ! alliance_is_inherit( $alliance_scheme ) ) {
		echo ' scheme_' . esc_attr( $alliance_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( alliance_get_theme_option( 'front_page_features_paddings' ) );
	if ( alliance_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$alliance_css      = '';
		$alliance_bg_image = alliance_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$alliance_anchor_icon = alliance_get_theme_option( 'front_page_features_anchor_icon' );
	$alliance_anchor_text = alliance_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( alliance_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$alliance_css      = '';
			$alliance_bg_mask  = alliance_get_theme_option( 'front_page_features_bg_mask' );
			$alliance_bg_color_type = alliance_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $alliance_bg_color_type ) {
				$alliance_bg_color = alliance_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$alliance_caption = alliance_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $alliance_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $alliance_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $alliance_caption, 'alliance_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$alliance_description = alliance_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $alliance_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $alliance_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $alliance_description ), 'alliance_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! alliance_exists_trx_addons() ) {
						...
						alliance_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
