...
if ( ! function_exists( 'alliance_storage_get' ) ) {
	/**
	 ...
	 */
	function alliance_storage_get( $var_name, $default = '' ) {
		global $ALLIANCE_STORAGE;
		return isset( $ALLIANCE_STORAGE[ $var_name ] ) ? $ALLIANCE_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'alliance_storage_set' ) ) {
	/**
	 ...
	 */
	function alliance_storage_set( $var_name, $value ) {
		global $ALLIANCE_STORAGE;
		$ALLIANCE_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'alliance_storage_empty' ) ) {
	/**
	 ...
	 */
	function alliance_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $ALLIANCE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $ALLIANCE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $ALLIANCE_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $ALLIANCE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'alliance_storage_isset' ) ) {
	/**
	 ...
	 */
	function alliance_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $ALLIANCE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $ALLIANCE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $ALLIANCE_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $ALLIANCE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'alliance_storage_unset' ) ) {
	/**
	 ...
	 */
	function alliance_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $ALLIANCE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $ALLIANCE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $ALLIANCE_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $ALLIANCE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'alliance_storage_inc' ) ) {
	/**
	 ...
	 */
	function alliance_storage_inc( $var_name, $value = 1 ) {
		global $ALLIANCE_STORAGE;
		if ( empty( $ALLIANCE_STORAGE[ $var_name ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ] = 0;
		}
		$ALLIANCE_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'alliance_storage_concat' ) ) {
	/**
	 ...
	 */
	function alliance_storage_concat( $var_name, $value ) {
		global $ALLIANCE_STORAGE;
		if ( empty( $ALLIANCE_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'alliance_storage_get_array' ) ) {
	/**
	 ...
	 */
	function alliance_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $ALLIANCE_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $ALLIANCE_STORAGE[ $var_name ][ $key ] ) ? $ALLIANCE_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $ALLIANCE_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $ALLIANCE_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'alliance_storage_set_array' ) ) {
	/**
	 ...
	 */
	function alliance_storage_set_array( $var_name, $key, $value ) {
		global $ALLIANCE_STORAGE;
		if ( ! isset( $ALLIANCE_STORAGE[ $var_name ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$ALLIANCE_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'alliance_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function alliance_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $ALLIANCE_STORAGE;
		if ( ! isset( $ALLIANCE_STORAGE[ $var_name ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $ALLIANCE_STORAGE[ $var_name ][ $key ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$ALLIANCE_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$ALLIANCE_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'alliance_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function alliance_storage_merge_array( $var_name, $key, $value ) {
		global $ALLIANCE_STORAGE;
		if ( ! isset( $ALLIANCE_STORAGE[ $var_name ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$ALLIANCE_STORAGE[ $var_name ] = array_merge( $ALLIANCE_STORAGE[ $var_name ], $value );
		} else {
			$ALLIANCE_STORAGE[ $var_name ][ $key ] = array_merge( $ALLIANCE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'alliance_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function alliance_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $ALLIANCE_STORAGE;
		if ( ! isset( $ALLIANCE_STORAGE[ $var_name ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			alliance_array_insert_after( $ALLIANCE_STORAGE[ $var_name ], $after, $key );
		} else {
			alliance_array_insert_after( $ALLIANCE_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'alliance_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function alliance_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $ALLIANCE_STORAGE;
		if ( ! isset( $ALLIANCE_STORAGE[ $var_name ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			alliance_array_insert_before( $ALLIANCE_STORAGE[ $var_name ], $before, $key );
		} else {
			alliance_array_insert_before( $ALLIANCE_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'alliance_storage_push_array' ) ) {
	/**
	 ...
	 */
	function alliance_storage_push_array( $var_name, $key, $value ) {
		global $ALLIANCE_STORAGE;
		if ( ! isset( $ALLIANCE_STORAGE[ $var_name ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $ALLIANCE_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $ALLIANCE_STORAGE[ $var_name ][ $key ] ) ) {
				$ALLIANCE_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $ALLIANCE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'alliance_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function alliance_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $ALLIANCE_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $ALLIANCE_STORAGE[ $var_name ] ) && is_array( $ALLIANCE_STORAGE[ $var_name ] ) && count( $ALLIANCE_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $ALLIANCE_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $ALLIANCE_STORAGE[ $var_name ][ $key ] ) && is_array( $ALLIANCE_STORAGE[ $var_name ][ $key ] ) && count( $ALLIANCE_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $ALLIANCE_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'alliance_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function alliance_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $ALLIANCE_STORAGE;
		if ( ! isset( $ALLIANCE_STORAGE[ $var_name ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $ALLIANCE_STORAGE[ $var_name ][ $key ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ][ $key ] = 0;
		}
		$ALLIANCE_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'alliance_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function alliance_storage_concat_array( $var_name, $key, $value ) {
		global $ALLIANCE_STORAGE;
		if ( ! isset( $ALLIANCE_STORAGE[ $var_name ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $ALLIANCE_STORAGE[ $var_name ][ $key ] ) ) {
			$ALLIANCE_STORAGE[ $var_name ][ $key ] = '';
		}
		$ALLIANCE_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'alliance_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function alliance_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $ALLIANCE_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $ALLIANCE_STORAGE[ $var_name ] ) ? $ALLIANCE_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $ALLIANCE_STORAGE[ $var_name ] ) ? $ALLIANCE_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'alliance_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function alliance_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $ALLIANCE_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $ALLIANCE_STORAGE[ $var_name ]->$prop ) ? $ALLIANCE_STORAGE[ $var_name ]->$prop : $default;
	}
}
