...
if ( ! function_exists( 'alliance_get_list_icons' ) ) {
	 ...
	 */
	function alliance_get_list_icons( $style = 'icons', $prepend_inherit = false ) {
		$lists = get_transient( 'alliance_list_icons' );
		if ( ! is_array( $lists ) || ! isset( $lists[ $style ] ) || ! is_array( $lists[ $style ] ) || count( $lists[ $style ] ) < 2 ) {
			if ( 'icons' == $style ) {
				$lists[ $style ] = alliance_array_from_list( alliance_get_list_icons_classes( $prepend_inherit ) );
			} elseif ( 'images' == $style ) {
