...
<?php
	...
	if ( apply_filters( 'alliance_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {		
		...
		// Featured image
		$show_featured = is_singular( 'attachment' ) && in_array( get_post_mime_type(get_the_ID()), array('audio/mpeg', 'video/quicktime') ) ? false : true; 
		$thumb_bg = in_array($alliance_post_format, array('audio', 'gallery', 'video')) || is_singular( 'attachment' ) ? false : true;
		$thumb_bg = 'audio' == $alliance_post_format && empty( alliance_get_post_audio_list() ) ? true : $thumb_bg;

		if ( $show_featured ) {
			ob_start();
			alliance_show_post_featured_image( array(
				'thumb_bg'  => $thumb_bg,
				'class'     => 'alignwide' . ($thumb_bg ? ' post_featured_bg' : ''),
				'popup'     => true,
	...
	<div class="post_footer post_footer_single entry-footer"><?php
		...
		if ( is_single() && ! is_attachment() ) {
			alliance_show_post_footer('pages,tags,share,prev_next,author');
		}
	?></div>
	<?php
