<div class="front_page_section front_page_section_contacts<?php
	$alliance_scheme = alliance_get_theme_option( 'front_page_contacts_scheme' );
	if ( ! empty( $alliance_scheme ) && ! alliance_is_inherit( $alliance_scheme ) ) {
		echo ' scheme_' . esc_attr( $alliance_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( alliance_get_theme_option( 'front_page_contacts_paddings' ) );
	if ( alliance_get_theme_option( 'front_page_contacts_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$alliance_css      = '';
		$alliance_bg_image = alliance_get_theme_option( 'front_page_contacts_bg_image' );
...
<?php
	// Add anchor
	$alliance_anchor_icon = alliance_get_theme_option( 'front_page_contacts_anchor_icon' );
	$alliance_anchor_text = alliance_get_theme_option( 'front_page_contacts_anchor_text' );
...
?>
	...
	<?php
	if ( alliance_get_theme_option( 'front_page_contacts_fullheight' ) ) {
	...
	?>
			...
			<?php
			$alliance_css      = '';
			$alliance_bg_mask  = alliance_get_theme_option( 'front_page_contacts_bg_mask' );
			$alliance_bg_color_type = alliance_get_theme_option( 'front_page_contacts_bg_color_type' );
			if ( 'custom' == $alliance_bg_color_type ) {
				$alliance_bg_color = alliance_get_theme_option( 'front_page_contacts_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_contacts_content_wrap content_wrap">
			...
			$alliance_caption     = alliance_get_theme_option( 'front_page_contacts_caption' );
			$alliance_description = alliance_get_theme_option( 'front_page_contacts_description' );
			if ( ! empty( $alliance_caption ) || ! empty( $alliance_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				...
				if ( ! empty( $alliance_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
					?>
					<h2 class="front_page_section_caption front_page_section_contacts_caption front_page_block_<?php echo ! empty( $alliance_caption ) ? 'filled' : 'empty'; ?>">
					<?php
				...
				if ( ! empty( $alliance_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
					?>
					<div class="front_page_section_description front_page_section_contacts_description front_page_block_<?php echo ! empty( $alliance_description ) ? 'filled' : 'empty'; ?>">
					<?php
			...
			// Content (text)
			$alliance_content = alliance_get_theme_option( 'front_page_contacts_content' );
			$alliance_layout  = alliance_get_theme_option( 'front_page_contacts_layout' );
			...
			if ( ( ! empty( $alliance_content ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) ) {
				?>
				<div class="front_page_section_content front_page_section_contacts_content front_page_block_<?php echo ! empty( $alliance_content ) ? 'filled' : 'empty'; ?>">
					<?php
			...
			if ( ! empty( $alliance_sc ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_output front_page_section_contacts_output front_page_block_<?php echo ! empty( $alliance_sc ) ? 'filled' : 'empty'; ?>">
					<?php
