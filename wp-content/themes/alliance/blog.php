...
if ( function_exists( 'alliance_elementor_is_preview' ) && alliance_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			alliance_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	alliance_new_main_query(
		array(
			'post_type'      => alliance_get_theme_option( 'post_type' ),
			'category'       => alliance_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'alliance_filter_get_template_part', alliance_blog_archive_get_template() ) );
}
